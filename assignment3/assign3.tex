...
\begin{align}
\therefore \pr{X=k}=\frac{1}{8}
\end{align}
Let $F_{X}\brak{k}$ be the Cumulative distribution function(CDF) such that;
\begin{align}
F_{X} \brak{k} &= P(X \leq k)\\
&=
\begin{cases}
0, & k \le 0\\
\frac{k}{8} & 1 \le k \le 8\\
1, & k \ge 9
\end{cases}
\end{align}
\begin{enumerate}
\item 
For k = 8, required probability is equivalent to;
\begin{align}
\pr{X=8} &= \frac{1}{8}\\
&= 0.125
\end{align}
...
&= 0.5
\end{align}
\item For k greater than 2:
\begin{align}
\pr{X > 2} &= 1 - \pr{X \le 2}\\
&= 1 - \brak{F_{X}\brak{2}-F_{X}\brak{0}}\\
&= \frac{6}{8}= 0.75
\end{align}
\item For k less than 9:
\begin{align}
